...
async function installDemoBridge(page) {
  await page.evaluate(({ prompts }) => {
    const legacyButton = document.querySelector('[data-demo-action="run"]');
    if (!legacyButton) throw new Error('未找到运行完整演示按钮');

    const runButton = legacyButton.cloneNode(true);
    runButton.dataset.rty798Bridge = 'true';
    legacyButton.replaceWith(runButton);
    ...
    runButton.addEventListener('click', (event) => {
      ...
      const actionButtons = [...document.querySelectorAll('.demo-mode-actions button')];
      ...
      const setActionsDisabled = (disabled) => actionButtons.forEach((button) => { button.disabled = disabled; });
...
async function testUiDevice(browser, device) {
  ...
  try {
    ...
    assert((await page.locator('.demo-mode-log li.success').count()) >= 7, 'demo success log is incomplete');
